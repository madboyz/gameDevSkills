---
name: engine-debug
description: 引擎源码调试与问题定位的规范和指南。业务侧出现异常时须先在引擎源码中归因根因，再向上给出业务/框架层修改建议。
---
...
> [!IMPORTANT]
> **凡表现为业务异常、界面不对、流程卡死、表现与策划不符等问题，默认先按「可能与引擎契约或调用时机有关」处理，禁止在未核对引擎源码前直接断定「纯业务写错」并只改业务层。**

**强制顺序**：

1. **先在 `scripting/engine` 中归因**：根据堆栈、涉及的组件/API（渲染、资源、节点生命周期、物理、动画等），阅读对应源码，明确**引擎实际行为、前置条件、调用顺序与生命周期约束**。
2. **再判定根因层级**：根因是引擎 Bug/版本差异、业务未满足引擎契约、还是框架封装与引擎不一致——结论须能**指向具体源码位置或引擎设计意图**。
3. **向上输出修改建议**：在完成第 1、2 步后，再给出对业务代码、项目框架层、资源或配置的修改建议；说明每条建议**依据哪段引擎源码或何种引擎契约**，避免无依据的「猜着改业务」。

仅当已通过引擎源码核对，确认与引擎行为无关（例如纯配置表填错、协议字段解析与引擎无关等），才可跳过引擎深挖，直接在对应层级修复。
...
在开始调试前，区分**现象**与**根因**：业务上的表现（闪退、花屏、资源未显示等）往往同时涉及业务代码与引擎；**应优先用引擎源码校准「引擎到底做了什么」**，再判断业务是否调用不当。

#### ✅ 需要先查引擎源码的场景（含大量「看起来像业务问题」的情况）

- 错误堆栈指向 `node_modules/cc` 或引擎内部模块
- API 调用结果与官方文档不一致，或与策划预期不一致（**用源码验证真实行为**）
- 控制台打印 Cocos Creator 内部警告/错误
- 涉及渲染、物理、动画、资源加载、节点/组件生命周期等——**即使堆栈只在业务脚本，也应查相关引擎 API 的实现与约定**
- **业务侧描述的「逻辑对但表现不对」**：先查引擎该功能链路的实现，再谈业务改法
...
以下类型**仍建议至少对照相关引擎 API 文档或源码中的契约**；若已确认与引擎无关，再只在业务/数据层修改：

- 经引擎源码核对后，确认为与引擎调用无关的纯数据错误（如配表 ID 引用错误）
- 与 Cocos 运行时无关的网络、服务端协议字段错误（不涉及引擎 API 链）
- 经核对后确认为项目内纯业务状态机/数值公式错误，且不依赖被误解的引擎行为

**禁止**：仅凭「像是业务逻辑问题」就跳过 `scripting/engine`，直接大规模改业务代码。
...
- [ ] **业务异常类问题**已先在 `scripting/engine` 中完成归因（或已书面说明为何与引擎无关），再给出对上层的修改建议
- [ ] 引擎相关的错误已通过查看 `scripting/engine` 源码定位根因
- [ ] 理解了引擎 API 的内部实现逻辑
- [ ] 业务代码调用方式符合引擎设计预期（且该结论有源码或文档依据）
- [ ] 已移除临时添加的引擎调试代码
- [ ] 已通过官方文档验证 API 使用正确性
- [ ] 已考虑引擎版本兼容性问题
...
> **引擎源码是理解引擎行为的唯一权威来源**
> 
> - 业务侧出问题，**先**在 `scripting/engine` 里把根因说清楚，**再**向上提业务/框架层改法
> - 遇到问题时，优先查看引擎源码实现
> - 不要猜测引擎行为，用源码验证假设
> - 理解引擎设计意图，而不是绕过引擎限制
> - 发现引擎 Bug 时，考虑向官方社区反馈

**记住**：`scripting/engine` 是你调试引擎问题的最佳伙伴；业务表现异常时，它是归因的第一站，而不是可选项。
